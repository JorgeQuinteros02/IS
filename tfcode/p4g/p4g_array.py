...
class P4GArray(MatrixGArray):
    ...
    def __init__(self, data, p='int'):
        ...
        self._left_actions[P4GArray] = self.__class__.left_action_hmat
        self._left_actions[Z2Array] = self.__class__.left_action_hvec

        super(P4GArray, self).__init__(data, p)
...
def identity(shape=(), p='int'):
    e = P4GArray(np.zeros(shape + (4,), dtype=np.int64), 'int')
...
def rand(minu, maxu, minv, maxv, size=()):
    ...
    return P4GArray(data=data, p='int')
...
def meshgrid(m=m_range(), r=r_range(), u=u_range(), v=v_range()):
    m = P4GArray(m.data[:, None, None, None, ...], p=m.p)
    r = P4GArray(r.data[None, :, None, None, ...], p=r.p)
    u = P4GArray(u.data[None, None, :, None, ...], p=u.p)
    v = P4GArray(v.data[None, None, None, :, ...], p=v.p)
    return u * v * m * r
