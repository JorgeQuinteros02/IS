...
class P4MArray(MatrixGArray):
    ...
    def hmat2int(self, hmat_data):
        ...
        out = np.zeros(hmat_data.shape[:-2] + (4,), dtype = np.dtype('int64'))
        out[..., 0] = m
        out[..., 1] = r
        out[..., 2] = u
        out[..., 3] = v
...
def identity(shape=(), p='int'):
    e = P4MArray(np.zeros(shape + (4,), dtype = np.dtype('int64')), 'int')
...
def rotation(r, center=(0, 0)):
    ...
    rdata = np.zeros(r.shape + (4,), dtype = np.dtype('int64'))
    rdata[..., 1] = r
    r0 = P4MArray(rdata)

    tdata = np.zeros(center.shape[:-1] + (4,), dtype = np.dtype('int64'))
    tdata[..., 2:] = center
    t = P4MArray(tdata)
...
def mirror_u(shape=None):
    ...
    mdata = np.zeros(shape + (4,), dtype = np.dtype('int64'))
    mdata[0] = 1
...
def m_range(start=0, stop=2):
    ...
    assert start < 2
    assert start < stop
    m = np.zeros((stop - start, 4), dtype = np.dtype('int64'))
