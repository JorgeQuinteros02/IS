...
class C4Array(MatrixGArray):
    ...
    def int2mat(self, int_data):
        r = int_data[..., 0]
        out = np.zeros(int_data.shape[:-1] + (2, 2), dtype = np.dtype('int64'))
    ...
    def mat2int(self, mat_data):
        ...
        out = np.zeros(mat_data.shape[:-2] + (1,), dtype = np.dtype('int64'))
        out[..., 0] = r
        return out
...
def identity(shape=(), p='int'):
    e = C4Array(np.zeros(shape + (1,), dtype = np.dtype('int64')), 'int')
    return e.reparameterize(p)


def rand(size=()):
    data = np.zeros(size + (1,), dtype=np.dtype('int64'))
