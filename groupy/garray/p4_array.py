...
class P4Array(MatrixGArray):
    ...
    def int2hmat(self, int_data):
        ...
        u = int_data[..., 1]
        v = int_data[..., 2]
        out = np.zeros(int_data.shape[:-1] + (3, 3), dtype = np.dtype('int64'))
    ...
    def hmat2int(self, mat_data):
        ...
        out = np.zeros(mat_data.shape[:-2] + (3,), dtype = np.dtype('int64'))
        out[..., 0] = r
        out[..., 1] = u
        out[..., 2] = v
        return out
...
def identity(shape=(), p='int'):
    e = P4Array(np.zeros(shape + (3,), dtype = np.dtype('int64')), 'int')
...
def rotation(r, center=(0, 0)):
    ...
    rdata = np.zeros(r.shape + (3,), dtype = np.dtype('int64'))
    rdata[..., 0] = r
    r0 = P4Array(rdata)

    tdata = np.zeros(center.shape[:-1] + (3,), dtype = np.dtype('int64'))
    tdata[..., 1:] = center
    t = P4Array(tdata)
...
def translation(t):
    t = np.asarray(t)
    tdata = np.zeros(t.shape[:-1] + (3,), dtype = np.dtype('int64'))
    tdata[..., 1:] = t
...
def r_range(start=0, stop=4, step=1):
    ...
    assert start < 4
    assert start < stop
    m = np.zeros((stop - start, 3), dtype = np.dtype('int64'))
